...
test("member POST join rejects missing invite code when configured", async () => {
  await withEnv({ LIMOS_JOIN_CODE: "team-code" }, async () => {
    await withSupabaseEnv(async () => {
      await withMockFetch(() => {
        return supabaseResponse([{
          payload: {
        ...
        assert.deepEqual(response.json(), { error: "invalid_invite_code" });
...
test("member POST join accepts configured invite code", async () => {
  await withEnv({ LIMOS_JOIN_CODE: "team-code" }, async () => {
    await withSupabaseEnv(async () => {
      await withMockFetch((call) => {
        if (call.options.method === "PATCH") {
